...
If you have entered a [Commicvine API Key](../app-settings/metadata.md) in settings, you'll see an additional icon in the File Manager. Clicking the <i class="bi bi-book fs-2 text-success"></i> icon will beging searching for metadata from ComicVine.
...
Additional settings can be found in the [Metadata Settings](../app-settings/metadata.md).
...
If this feature is enabled, when a `ComicInfo.xml` file is generated by ComicVine, the file will be renamed to match your configured [Custom Rename Pattern](../app-settings/file-settings.md#custom-naming-settings).