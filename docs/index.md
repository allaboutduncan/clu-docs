---
hide:
  - navigation
  - toc
---

<div class="hero-section">
  <div class="hero-content">
    <h1>Your Comics, Your Way</h1>
    <p>The ultimate self-hosted solution for organizing, reading, and managing your digital comic collection.</p>
    
    <div class="hero-badges">
      <img src="https://img.shields.io/docker/pulls/allaboutduncan/comic-utils-web" alt="Docker Pulls">
      <img src="https://img.shields.io/github/v/release/allaboutduncan/comic-utils" alt="GitHub Release">
      <img src="https://img.shields.io/github/commits-since/allaboutduncan/comic-utils/latest" alt="GitHub commits">
    </div>

    <a href="getting-started/quickstart/" class="cta-button">Get Started</a>
  </div>

  <div class="video-container">
    <!-- Replace with actual video embed or image -->
    <div style="padding: 2rem; text-align: center;">
      <svg style="width:64px;height:64px;margin-bottom:1rem;" viewBox="0 0 24 24">
        <path fill="currentColor" d="M10,16.5V7.5L16,12M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2Z" />
      </svg>
      <h3>Introduction to CLU</h3>
      <p>Watch the overview video</p>
    </div>
  </div>
</div>

<div class="feature-grid">

  <div class="feature-card">
    <span class="feature-icon">📚</span>
    <h3>Smart Library</h3>
    <p>Automatically organize your comics with metadata scraping and smart folder monitoring. Keep your collection pristine effortlessly.</p>
  </div>

  <div class="feature-card">
    <span class="feature-icon">🔍</span>
    <h3>Deep Search</h3>
    <p>Find any issue instantly with powerful search and filtering. Sort by publisher, year, character, or story arc.</p>
  </div>

  <div class="feature-card">
    <span class="feature-icon">📱</span>
    <h3>Read Anywhere</h3>
    <p>Stream your comics to any device. Built-in web reader supports CBZ, CBR, PDF, and more with seamless progress syncing.</p>
  </div>

  <div class="feature-card">
    <span class="feature-icon">🔄</span>
    <h3>Collection Tracking</h3>
    <p>Track your reading progress, manage want-to-read lists, and integrate with ComicVine for unparalleled data accuracy.</p>
  </div>

</div>

<div class="content-section">
  <h2>How it Works</h2>
  <div class="steps-grid">
    <div class="step-card">
      <div class="step-number">1</div>
      <h3>Install</h3>
      <p>Run CLU on your computer, server, or NAS via Docker or Python.</p>
    </div>
    <div class="step-card">
      <div class="step-number">2</div>
      <h3>Map</h3>
      <p>Point CLU to your existing comic library folder location.</p>
    </div>
    <div class="step-card">
      <div class="step-number">3</div>
      <h3>Manage</h3>
      <p>Access the web interface to organize, edit, and enhance your collection.</p>
    </div>
    <div class="step-card">
      <div class="step-number">4</div>
      <h3>Enjoy</h3>
      <p>Read your comics on any device with the built-in reader.</p>
    </div>
  </div>
</div>

<div class="content-section alt-bg">
  <h2>Supported Media</h2>
  <div class="media-grid">
    <div class="media-card">
      <h3>File Formats</h3>
      <ul>
        <li><strong>CBZ</strong> / ZIP</li>
        <li><strong>CBR</strong> / RAR</li>
        <li><strong>PDF</strong> Documents</li>
      </ul>
    </div>
    <div class="media-card">
      <h3>Image Processing</h3>
      <ul>
        <li><strong>JPG</strong> / JPEG</li>
        <li><strong>PNG</strong> Images</li>
        <li><strong>GIF</strong> Animations</li>
      </ul>
    </div>
  </div>
</div>
